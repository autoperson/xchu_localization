...
pcl::PointCloud<pcl::PointXYZ>::Ptr part_origin;
...
sensor_msgs::PointCloud2 create_pcd(const std::vector<std::string> &pcd_paths, int *ret_err = NULL) {
  ...
  for (const std::string &path : pcd_paths) {
    ...
    if (pcd.width == 0) { // size = 0
      ...
      part_origin.reset(new pcl::PointCloud<pcl::PointXYZ>());
      if (pcl::io::loadPCDFile(path.c_str(), *part_origin) == -1) {
        std::cerr << "load failed " << path << std::endl;
        if (ret_err)
          *ret_err = 1;
      }
      // downsample pcd
      std::cout << "origin pcd map : " << part_origin->size() << std::endl;
      boost::shared_ptr<pcl::VoxelGrid<pcl::PointXYZ>> voxelgrid(new pcl::VoxelGrid<pcl::PointXYZ>());
      voxelgrid->setLeafSize(downsample_resolution, downsample_resolution, downsample_resolution);
      voxelgrid->setInputCloud(part_origin);

      pcl::PointCloud<pcl::PointXYZ>::Ptr filtered(new pcl::PointCloud<pcl::PointXYZ>());
      voxelgrid->filter(*filtered);

      pcl::toROSMsg(*filtered, part);
      std::cout << "filtered pcd map : " << part.width << std::endl;
...
//        }
  ...
  // points map节点
  pcd_pub = nh.advertise<sensor_msgs::PointCloud2>("points_map", 5, true);
  ...
  if (margin < 0) {
    ...
    // 接收到不同的位置消息去发布points map
    gnss_sub = nh.subscribe("gnss_pose", 5, publish_gnss_pcd);
    current_sub = nh.subscribe("current_pose", 5, publish_current_pcd);
