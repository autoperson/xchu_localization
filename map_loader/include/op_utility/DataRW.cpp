...
namespace UtilityHNS {
    ...
    void DataRW::WriteKMLFile(const string &fileName, const vector <string> &gps_list) {
        ...
        pV = hKmlFile.FirstChild("Folder").FirstChild("Document").FirstChild("Placemark").FirstChild(
                "LineString").FirstChild("coordinates").Element();
        if (!pV)
...
//Points
    ...
    AisanPointsFileReader::AisanPointsFileReader(const vector_map_msgs::PointArray &_points) : SimpleReaderBase("d",
                                                                                                                1) {
...
//Lane
    ...
    AisanLanesFileReader::AisanLanesFileReader(const vector_map_msgs::LaneArray &_lanes) : SimpleReaderBase("d", 1) {
        ...
        for (unsigned int i = 0; i < _lanes.data.size(); i++) { //  每一条lane记录
            ParseNextLine(_lanes.data.at(i), data); // msg -> AisanLane

            m_data_list.push_back(data);
            if (data.LnID < m_min_id)
                m_min_id = data.LnID; // lane的最小id

            if (data.LnID > max_id)
                max_id = data.LnID;  // lane的最大id
        }

        m_data_map.resize(max_id - m_min_id + 2); // lane的size，直接_lanes.data.size()不就可以了吗？
        for (unsigned int i = 0; i < m_data_list.size(); i++) {
...
//RoadSign
    ...
    AisanRoadSignFileReader::AisanRoadSignFileReader(const vector_map_msgs::RoadSignArray &_signs) : SimpleReaderBase(
            "d", 1) {
...
//Signal
    ...
    AisanSignalFileReader::AisanSignalFileReader(const vector_map_msgs::SignalArray &_signal) : SimpleReaderBase("d",
                                                                                                                 1) {
...
//Vector
    ...
    AisanVectorFileReader::AisanVectorFileReader(const vector_map_msgs::VectorArray &_vectors) : SimpleReaderBase("d",
                                                                                                                  1) {
...
// RoadEdge
    ...
    AisanRoadEdgeFileReader::AisanRoadEdgeFileReader(const vector_map_msgs::RoadEdgeArray &_edges) : SimpleReaderBase(
            "d", 1) {
...
//WayArea
    ...
    AisanWayareaFileReader::AisanWayareaFileReader(const vector_map_msgs::WayAreaArray &_wayAreas) : SimpleReaderBase(
            "d", 1) {
