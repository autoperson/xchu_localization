...
namespace SC2 {
    ...
    MatrixXd SCManager::makeScancontext(pcl::PointCloud<SCPointType> &_scan_down) {
        ...
        MatrixXd desc = NO_POINT * MatrixXd::Ones(PC_NUM_RING, PC_NUM_SECTOR);  //  初始化ring*sector的矩阵

        cv::Mat rangeMat(PC_NUM_RING, PC_NUM_SECTOR, CV_32F, cv::Scalar::all(FLT_MAX));
        //cv::Mat rangeMat_int(PC_NUM_RING, PC_NUM_SECTOR, CV_16UC1);
        ...
        for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++) {
            ...
            // xyz to ring, sector
            azim_range = sqrt(pt.x * pt.x + pt.y * pt.y);  // 深度
            azim_angle = xy2theta(pt.x, pt.y);  //  水平转过的角度
            ...
            if (desc(ring_idx - 1, sctor_idx - 1) < pt.z) // -1 means cpp starts from 0
            {
                ...
                // 生成图像
                rangeMat.at<float>(ring_idx - 1, sctor_idx - 1) = pt.z;
                //rangeMat_int.at<ushort>(ring_idx - 1, sctor_idx - 1) = round(sqrt(pt.z) * 1000);
            }
        }

        // reset no points to zero (for cosine dist later) 没有点的话深度设置为0
        for (int row_idx = 0; row_idx < desc.rows(); row_idx++) {
            for (int col_idx = 0; col_idx < desc.cols(); col_idx++) {
                if (desc(row_idx, col_idx) == NO_POINT) {
                    desc(row_idx, col_idx) = 0;
                    rangeMat.at<float>(ring_idx - 1, sctor_idx - 1) = 0;
                    //rangeMat_int.at<ushort>(ring_idx - 1, sctor_idx - 1) = 0;
                }
            }
        }
        //cv::imwrite("/home/xchu/data/yiqingchuangxin_map2.0.0/Raw/pcd_buffer/sc_images/" + std::to_string(i) + ".jpg",
         //           rangeMat);
        i++;
        //cv::imshow("image", rangeMat);
        // cv::waitKey(0);
        std::cout << "write image:" << std::to_string(i) + "_sc.png" << std::endl;
        //std::cout << "range image:" << rangeMat.size << std::endl;
    ...
    void SCManager::makeAndSaveScancontextAndKeys(pcl::PointCloud<SCPointType> &_scan_down) {
        ...
        Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1  20*60
        Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(sc); // ring key (ring索引)  20*1
        Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(sc); // sector key (区块索引) 60*1
        std::vector<float> polarcontext_invkey_vec = eig2stdvec(ringkey);  // vector形式的ring key
