...
namespace SC2 {
    ...
    public:
        // hyper parameters ()
        const double LIDAR_HEIGHT = 1.6; // lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord) / if you use robot-coord-transformed lidar scans, just set this as 0.

        const int PC_NUM_RING = 100; // 20 in the original paper (IROS 18)
        const int PC_NUM_SECTOR = 300; // 60 in the original paper (IROS 18)
        ...
        const double SC_DIST_THRES = 0.4; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold
