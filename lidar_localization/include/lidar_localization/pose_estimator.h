...
class PoseEstimator {
 public:
  ...
  std::unique_ptr</*kalman_filter::*/UnscentedKalmanFilter<float, PoseSystem>> ukf;
...
*/
  PoseEstimator(/*pcl::Registration<PointT, PointT>::Ptr*/
      pclomp::NormalDistributionsTransform<PointT, PointT>::Ptr
      &registration, const ros::Time &stamp,
    ...
    PoseSystem system;
    ukf.reset(new /*kalman_filter::*/UnscentedKalmanFilter<float, PoseSystem>(system, 16, 6, 7, process_noise,
                                                                          measurement_noise, mean, cov));
  }
  ...
  void predict(const ros::Time &stamp, const Eigen::Vector3f &acc, const Eigen::Vector3f &gyro) {
    ...
    ukf->setProcessNoiseCov(process_noise * dt); //  过程噪声
    ukf->system.dt = dt;
  ...
  pcl::PointCloud<PointT>::Ptr correct(const pcl::PointCloud<PointT>::ConstPtr &cloud) {
    ...
    Eigen::Matrix4f trans = registration->getFinalTransformation();// ndt的结果作为观测去更新测量
  ...
  Eigen::Matrix4f init_matrix() const {
    if (init_guess == Eigen::Matrix4f::Identity()) {
      std::cout << "init 0 matix .." << std::endl;
    }
    return init_guess;
  }
