...
class UnscentedKalmanFilter {
 ...
 public:
   ...
   */
  UnscentedKalmanFilter(const System &system, int state_dim, int input_dim, int measurement_dim,
  ...
  void correct(const VectorXt &measurement) {
    ...
    for (int i = 0; i < ext_sigma_points.rows(); i++) {
      ...
      expected_measurements.row(i) += VectorXt(
          ext_sigma_points.row(i).transpose().bottomRightCorner(K, 1));
    }
  ...
  /*	setter	*/
  UnscentedKalmanFilter &setMean(const VectorXt &m) {
    mean = m;
    return *this;
  }

  UnscentedKalmanFilter &setCov(const MatrixXt &s) {
    cov = s;
    return *this;
  }

  UnscentedKalmanFilter &setProcessNoiseCov(const MatrixXt &p) {
    process_noise = p;
    return *this;
  }

  UnscentedKalmanFilter &setMeasurementNoiseCov(const MatrixXt &m) {
    measurement_noise = m;
    return *this;
  }
